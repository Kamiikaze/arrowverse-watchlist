<template>
    <v-form v-model="form" class="pa-4" @submit.prevent>
        <v-alert v-if="alert" :type="alert.type" class="mb-4" closable>
            <p v-if="alert.code === 'fbAuth/email-not-verified'">
                <v-btn
                    class="ma-0 pa-0 mt-4 text-decoration-underline"
                    density="compact"
                    variant="text"
            ...
            <p v-if="alert.code === 'fbAuth/user-not-found'">
                <span>Don't have an account yet? <br /> </span>
                <v-btn
                    class="ma-0 pa-0 mt-4 text-decoration-underline"
                    density="compact"
                    variant="text"
                    @click="this.$emit('changeTab', 'register')"
                    >Register
                </v-btn>
            </p>
        ...
        >
            <v-card-text class="text-medium-emphasis text-caption">
                ...
                now, you can also click "Forgot login password?" to reset the
                login password.
            </v-card-text>
        </v-card>
...
export default {
    ...
    methods: {
        ...
        doUserAuth() {
            ...
            if (this.method === 'register') {
                ...
                this.loginUser(this.userAuth.email, this.userAuth.pw).then(
                    (login) => {
                        if (login) {
                            this.toggleDialog()
                        }
    ...
    computed: {
        ...
        passwordMatch() {
            return (
                this.userAuth.pw === this.userAuth.pwconfirm ||
